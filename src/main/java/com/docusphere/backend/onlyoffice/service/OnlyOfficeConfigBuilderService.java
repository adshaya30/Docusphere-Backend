...
public class OnlyOfficeConfigBuilderService {
    ...
    /**
     * Builds a read-only ONLYOFFICE configuration for a historical document version.
     * Does not register a save callback — preview never mutates storage or creates versions.
     */
    public OnlyOfficeConfig buildVersionPreviewConfig(
        ...
        OnlyOfficeConfig.GoBack goBack = OnlyOfficeConfig.GoBack.builder()
                .url(frontendUrl + "/documents/" + document.getId() + "/versions")
                .build();
