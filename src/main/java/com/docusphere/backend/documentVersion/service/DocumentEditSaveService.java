...
public class DocumentEditSaveService {
    ...
    private final DocumentVersionIdempotencyGuard idempotencyGuard;
    ...
    public void processOnlyOfficeSave(
        ...
        processOnlyOfficeSave(documentId, editedBy, fileBytes, idempotencyKey, null);
    }
    ...
    public void processOnlyOfficeSave(
        ...
        if (idempotencyGuard.shouldSkipDuplicateSave(documentId, idempotencyKey)) {
            log.info("Skipping duplicate ONLYOFFICE save for document {}", documentId);
            applyPendingSummaryToLatestVersion(documentId, editedBy);
            return;
        }
        ...
        idempotencyGuard.recordSave(documentId, idempotencyKey);
    ...
    private Optional<DocumentVersion> createVersionSnapshotIfNeeded(
        ...
        return createVersionSnapshotIfNeeded(
                document, editedBy, changeSummary, idempotencyKey, priorUpdatedAt,
                markCurrent, markRestored, forceCreate, fileBytesOverride, sourceStorageKeyOverride, null
        );
    }
    ...
    private Optional<DocumentVersion> createVersionSnapshotIfNeeded(
        ...
        if (!forceCreate && shouldSkipSnapshot(document.getId(), editedBy, idempotencyKey, priorUpdatedAt, editorEmail)) {
    ...
    private DocumentVersion createVersionSnapshot(
        ...
        idempotencyGuard.recordVersionCreation(
                document.getId(),
                editedBy,
                idempotencyKey,
                document.getUpdatedAt()
        ...
        recordAudit("VERSION_CREATED", document.getId(), saved.getId(), editedBy, Map.of(
                ...
                "invitedEmail", editorEmail != null ? editorEmail : "",
                "editorAttribution", editorEmail != null ? "Edited via shared invitation" : ""
        ));
    ...
    private boolean shouldSkipSnapshot(
        ...
        if (idempotencyGuard.shouldSkipVersionCreation(documentId, editedBy, idempotencyKey, priorUpdatedAt)) {
            return true;
        }

        // Shared invitation edits always get their own version row.
    ...
    private void applyPendingSummaryToLatestVersion(UUID documentId, Long editedBy) {
        String pending = summaryStore.peek(documentId, editedBy);
        if (pending == null || pending.isBlank() || DEFAULT_CHANGE_SUMMARY.equals(pending.trim())) {
            return;
        }
        documentVersionRepository.findFirstByDocumentIdOrderByVersionNumberDesc(documentId)
                ...
                .ifPresent(version -> {
                    if (DEFAULT_CHANGE_SUMMARY.equals(version.getChangeSummary())) {
                        version.setChangeSummary(pending.trim());
                        documentVersionRepository.save(version);
                        log.info("Applied pending summary to version {} for document {}",
                                version.getVersionNumber(), documentId);
                    }
                });
    }
